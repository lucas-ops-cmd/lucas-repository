...
public class DatabaseConnection {
    ...
    public static List<Mission> getMissionsPers(int idPersonnel) {
        updateMissionsStatus(); // Changer le statut des missions
        List<Mission> missions = new ArrayList<>();
        String sql = "SELECT m.ID_Mission, Nom, Description, Date_Debut, Duree, Statut " +
    ...
    //Afficher les competences de la mission choisis
    ...
    public static String getCompetenceMission(int missionId) {
        String sql = "SELECT c.Nom_competence, c.Code_Competence " +
                "FROM necessiter n JOIN competence c ON n.Code_Competence = c.Code_Competence " +
        ...
        try (PreparedStatement stmt = getConnection().prepareStatement(sql)) {
            ...
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                if (!competencesList.isEmpty()) competencesList.append(", ");
                competencesList.append(rs.getString("Code_Competence")+" : "+rs.getString("Nom_competence"));
            }
        } catch (SQLException e) {
